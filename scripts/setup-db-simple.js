...
async function setupDatabase() {
  console.log('🚀 เริ่มต้นสร้างโครงสร้างฐานข้อมูล PR_PO\n');

  try {
    // 1. สร้างตาราง pr_master
    console.log('📝 กำลังสร้างตาราง pr_master...');
    ...
    `;
    console.log('✅ สร้างตาราง pr_master เสร็จสิ้น\n');

    // สร้าง indexes สำหรับ pr_master
    console.log('📝 กำลังสร้าง indexes สำหรับ pr_master...');
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_master_doc_num ON pr_master(doc_num)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_master_doc_date ON pr_master(doc_date)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_master_doc_status ON pr_master(doc_status)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_master_series_name ON pr_master(series_name)`;
    console.log('✅ สร้าง indexes เสร็จสิ้น\n');

    // 2. สร้างตาราง pr_lines
    console.log('📝 กำลังสร้างตาราง pr_lines...');
    await prisma.$executeRaw`
    ...
    `;
    console.log('✅ สร้างตาราง pr_lines เสร็จสิ้น\n');

    // สร้าง indexes สำหรับ pr_lines
    console.log('📝 กำลังสร้าง indexes สำหรับ pr_lines...');
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_lines_pr_doc_num ON pr_lines(pr_doc_num)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_lines_item_code ON pr_lines(item_code)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_lines_line_status ON pr_lines(line_status)`;
    console.log('✅ สร้าง indexes เสร็จสิ้น\n');

    // 3. สร้างตาราง pr_po_links
    console.log('📝 กำลังสร้างตาราง pr_po_links...');
    await prisma.$executeRaw`
    ...
    `;
    console.log('✅ สร้างตาราง pr_po_links เสร็จสิ้น\n');

    // สร้าง indexes สำหรับ pr_po_links
    console.log('📝 กำลังสร้าง indexes สำหรับ pr_po_links...');
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_po_links_pr_doc_num ON pr_po_links(pr_doc_num)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_pr_po_links_po_doc_num ON pr_po_links(po_doc_num)`;
    console.log('✅ สร้าง indexes เสร็จสิ้น\n');

    // 4. สร้างตาราง sync_log
    console.log('📝 กำลังสร้างตาราง sync_log...');
    await prisma.$executeRaw`
    ...
    `;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_sync_log_sync_date ON sync_log(sync_date)`;
    console.log('✅ สร้างตาราง sync_log เสร็จสิ้น\n');

    console.log('⏳ กำลังสร้าง Functions และ Views (อาจใช้เวลาสักครู่)...\n');

    // 5. สร้าง Function: upsert_pr_data
    console.log('📝 กำลังสร้าง function upsert_pr_data...');
    await prisma.$executeRaw`
      ...
      $$ LANGUAGE plpgsql
    `;
    console.log('✅ สร้าง function upsert_pr_data เสร็จสิ้น\n');

    // 6. สร้าง View: vw_pr_detail
    console.log('📝 กำลังสร้าง view vw_pr_detail...');
    await prisma.$executeRaw`
    ...
    `;
    console.log('✅ สร้าง view vw_pr_detail เสร็จสิ้น\n');

    // 7. สร้าง Materialized View: mv_pr_summary
    console.log('📝 กำลังสร้าง materialized view mv_pr_summary...');
    await prisma.$executeRaw`DROP MATERIALIZED VIEW IF EXISTS mv_pr_summary CASCADE`;
    await prisma.$executeRaw`
    ...
    `;
    console.log('✅ สร้าง materialized view mv_pr_summary เสร็จสิ้น\n');

    // สร้าง indexes สำหรับ materialized view
    console.log('📝 กำลังสร้าง indexes สำหรับ mv_pr_summary...');
    await prisma.$executeRaw`CREATE UNIQUE INDEX IF NOT EXISTS idx_mv_pr_summary_doc_num ON mv_pr_summary(doc_num)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_mv_pr_summary_doc_date ON mv_pr_summary(doc_date)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_mv_pr_summary_doc_status ON mv_pr_summary(doc_status)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_mv_pr_summary_series_name ON mv_pr_summary(series_name)`;
    await prisma.$executeRaw`CREATE INDEX IF NOT EXISTS idx_mv_pr_summary_is_complete ON mv_pr_summary(is_complete)`;
    console.log('✅ สร้าง indexes เสร็จสิ้น\n');

    // 8. สร้าง Function: quick_refresh_view
    console.log('📝 กำลังสร้าง function quick_refresh_view...');
    await prisma.$executeRaw`
      ...
      $$ LANGUAGE plpgsql
    `;
    console.log('✅ สร้าง function quick_refresh_view เสร็จสิ้น\n');
    ...
    console.log('\n📌 ขั้นตอนต่อไป:');
    console.log('   1. เปิด http://localhost:2025/pr-tracking');
    console.log('   2. กดปุ่ม "🔄 ซิงค์ข้อมูล" เพื่อดึงข้อมูลจาก SAP');
    console.log('   3. เริ่มใช้งานระบบ!\n');
